...
public abstract class RegistryMapConfig<X extends IForgeRegistryEntry<X>, Y> extends AbstractConfigValue<ImmutableMap<X, Y>> {
    ...
    private final ITagCollection<X> tagCollection;

    public RegistryMapConfig(String identifier, IForgeRegistry<X> registry, ITagCollection<X> tags, Map<X, Y> defaultValues) {
        super(identifier);
    ...
    public void deserialize(JsonObject jsonObject) {
        ...
        while (iterator.hasNext()) {
            ...
            if (entry.getKey().startsWith("#")) {
                ITag<X> tag = ConfigUtils.getTagKeyFromTag(entry.getKey(), tagCollection);
                if (tag != null) {
                    List<X> entries = tag.getValues();
                    this.addAllEntries(parsedValues, entries, value);
                    ToolLeveling.LOGGER.info("Found tag '{}' in '{}'", entry.getKey(), getIdentifier());
                    continue;
                }
