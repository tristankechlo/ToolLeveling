...
public final class ButtonHelper {
    ...
    public static IFormattableTextComponent getButtonText(ButtonEntry entry) {
    ...
    public static List<ITextComponent> getButtonToolTips(ButtonEntry data) {
        ...
        if (ButtonHelper.shouldButtonBeActive(data) || Utils.freeCreativeUpgrades(Minecraft.getInstance().player)) {
            tooltip.add(new TranslationTextComponent(start + ".current_level", data.currentLevel).withStyle(TextFormatting.DARK_GRAY));
            tooltip.add(new TranslationTextComponent(start + ".next_level", (data.currentLevel + 1)).withStyle(TextFormatting.DARK_GRAY));
            tooltip.add(new TranslationTextComponent(start + ".cost", data.upgradeCost).withStyle(TextFormatting.DARK_GRAY));
        }
        if (Utils.freeCreativeUpgrades(Minecraft.getInstance().player)) {
            tooltip.add(new TranslationTextComponent(start + ".free_creative").withStyle(TextFormatting.GREEN));
        } else if (data.getStatus() != ButtonStatus.NORMAL) {
            tooltip.add(new TranslationTextComponent(start + ".error." + data.getStatus().toString().toLowerCase()).withStyle(ButtonHelper.getButtonTextFormatting(data)));
        }
        return tooltip;
    }
