...
public class ItemValuesListWidget extends ObjectSelectionList<ItemValueEntry> {
    ...
    private void refreshList() {
        List<Tuple<ItemStack, Long>> entries = ItemValueConfig.get().itemValues().entrySet().stream()
        ...
        for (List<Tuple<ItemStack, Long>> list : partitions) {
            NonNullList<Tuple<ItemStack, Long>> nonNullList = NonNullList.withSize(partitionSize,
                    new Tuple<>(ItemStack.EMPTY, 0L));
